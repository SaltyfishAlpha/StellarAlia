...
// Requires set=2 bindings; only include where a skinned descriptor set is bound.

struct SkinVertex { uvec4 joints; vec4 weights; };
layout(std430, set = 2, binding = 0) readonly buffer SkinMatrices { mat4       u_Bones[];     };
layout(std430, set = 2, binding = 1) readonly buffer SkinData     { SkinVertex u_SkinVerts[]; };
