...
void SettingsPanel::OnDraw() {
    ...
    if (m_overlaySettings) {
        if (ImGui::CollapsingHeader("Overlay", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            ImGui::Checkbox("Point Light Range",     &m_overlaySettings->drawPointLightRange);
            ImGui::Checkbox("Spot Light Cone",       &m_overlaySettings->drawSpotLightCone);
            ImGui::Checkbox("Area Light Rect",       &m_overlaySettings->drawAreaLightRect);
            ImGui::Checkbox("Directional Light Dir", &m_overlaySettings->drawDirectionalLightDir);
            ImGui::Separator();
            ImGui::Separator();
            ImGui::Checkbox("Entity Icons",        &m_overlaySettings->drawEntityIcons);
            ImGui::BeginDisabled(!m_overlaySettings->drawEntityIcons);
            ImGui::SliderFloat("Icon Size", &m_overlaySettings->billboardIconSize, 16.f, 64.f, "%.0f px");
            ImGui::EndDisabled();
            ImGui::Separator();
