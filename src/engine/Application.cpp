...
bool Application::Init(const Desc& desc) {
    ...
    m_animSystem.Init(m_device.get(), m_renderer.GetSkinDescLayout(),
                      m_renderer.GetVelocityDescLayout());
...
void Application::UpdateProjectPaths(const std::filesystem::path& projectDir,
    ...
    InputMapLoader::LoadAll(projectDir, cookCacheDir, m_input);
}

void Application::ReloadInputMaps() {
    if (m_desc.projectDir.empty()) return;
    InputMapLoader::LoadAll(m_desc.projectDir, m_desc.cookCacheDir, m_input);
}
...
// Copy the public C# API surface (Runtime DLL + debug symbols + XML doc) from
// the engine build output into the project's Library/managed/ so the IDE-side
// HintPath in {stem}.csproj resolves relative to the project (Issue #82).
// ScriptBridge / Microsoft.CodeAnalysis are engine-internal and intentionally
// excluded — user scripts never reference them.
static void CopyManagedLibsToProject(const fs::path& projectDir,
                                      const fs::path& engineManagedDir) {
    std::error_code ec;
    if (!fs::is_directory(engineManagedDir, ec)) {
        SA_LOG_WARN("Application: engine managed dir missing — Library/managed not refreshed: '{}'",
                    engineManagedDir.string());
        return;
    }
    const fs::path dst = projectDir / "Library" / "managed";
    fs::create_directories(dst, ec);
    if (ec) {
        SA_LOG_WARN("Application: cannot create Library/managed: {}", ec.message());
        return;
    }
    static constexpr const char* kNames[] = {
        "StellarAlia.Runtime.dll",
        "StellarAlia.Runtime.xml",
        "StellarAlia.Runtime.pdb",
    };
    for (const char* name : kNames) {
        const fs::path src = engineManagedDir / name;
        if (!fs::exists(src, ec)) continue;   // .xml/.pdb may be absent in release builds
        fs::copy_file(src, dst / name, fs::copy_options::overwrite_existing, ec);
        if (ec)
            SA_LOG_WARN("Application: failed to copy {} -> Library/managed: {}",
                        name, ec.message());
    }
}

static std::string LoadGitignoreTemplate(const fs::path& engineAssetsDir) {
    const fs::path tmpl = engineAssetsDir / "templates" / "project" / ".gitignore.template";
    std::ifstream f(tmpl);
    if (!f) {
        // Fallback when engine assets are missing (e.g. running examples
        // without a populated engineAssetsDir) — keep the project usable.
        return "Library/\n.vs/\n.idea/\nobj/\nbin/\ncook_cache/\n";
    }
    std::stringstream ss;
    ss << f.rdbuf();
...
void Application::GenerateIdeProjectFiles(const fs::path& projectDir) {
    ...
    const fs::path engineManaged = fs::path(StellarAliaApp::BIN_DIR) / "managed";
    CopyManagedLibsToProject(projectDir, engineManaged);

    std::string stem = projectDir.stem().string();
    if (stem.empty()) stem = "Scripts";
    std::string guid = MakeGuidFromStem(stem);

    // ── Directory.Build.props — references local Library/managed (relative). ─
    // Identical content across machines; safe to commit.
    std::string props =
        "<Project>\n"
        "  <PropertyGroup>\n"
        "    <!-- Auto-generated by StellarAlia Editor — references local Library/managed. Safe to commit. -->\n"
        "    <StellarAliaManaged>$(MSBuildThisFileDirectory)Library\\managed</StellarAliaManaged>\n"
        "  </PropertyGroup>\n"
        "</Project>\n";
    {
    ...
    // ── .gitignore — write only if absent so user-customised ignores survive ─
    WriteFileIfMissing(projectDir / ".gitignore", LoadGitignoreTemplate(m_desc.engineAssetsDir));
}
